...
def execute_ft(
    ...
    **kwargs: Any,
    ...
    for it in range(hparams.num_steps):
        loss_meter.reset()
        batch_iterator = tqdm(
            zip(chunks(texts, hparams.batch_size), chunks(targets, hparams.batch_size)),
            desc=f"Epoch {it+1}/{hparams.num_steps}",
            leave=False,
            total=(len(texts) + hparams.batch_size - 1) // hparams.batch_size
        )
        for txt, tgt in batch_iterator:
            ...
            if 't5' in hparams.model_name.lower():
                ...
                if hparams.objective_optimization == 'prompt_last':
                    ...
                    loss = -(torch.gather(probs, 1, target_ids) * loss_mask).sum(1) / mask_sum
                    loss = loss.mean()
                elif hparams.objective_optimization == 'target_new':
                    ...
                    raise NotImplementedError
            batch_iterator.set_postfix({"Batch loss": f"{loss.item():.4f}"}, refresh=True)
